...
describe('Product Entity test', () => {
  ...
  test('Product 엔티티를 생성할 수 있다.', async () => {
    ...
    const actual = new Product().create(productName, description, price, isDisplay)
  ...
  test('Product 가격이 0보다 작으면 예외를 반환할 수 있다.', async () => {
    ...
    try {
      const product = new Product().create(productName, description, price, isDisplay)
      product.validatePrice()
    } catch (error) {
      expect(error.name).toBe('ValidationError')
      expect(error.message).toBe('가격 정보가 올바르지 않습니다.')
    }
  })
})
